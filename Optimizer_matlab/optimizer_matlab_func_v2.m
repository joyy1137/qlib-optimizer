...
try
    ...
    if exist(objective_weights_path, 'file')
        ...
        fprintf('从配置文件读取权重：分数权重=%.2f, 夏普比率权重=%.2f\n', alpha, beta);
    else
        % 默认权重
        alpha = 0.7;  % 分数权重
        beta = 0.3;   % 夏普比率权重
        fprintf('使用默认权重：分数权重=%.2f, 夏普比率权重=%.2f\n', alpha, beta);
    end
    ...
    min_eig = min(eig(V));
    if min_eig <= 0
        fprintf('协方差矩阵不是正定的，最小特征值: %.6f，添加正则化项\n', min_eig);
    ...
    if exist(yesterday_weight_path, 'file') && exist(yesterday_code_path, 'file')
        ...
        if length(yesterday_weight) == length(yesterday_codes)
            ...
            fprintf('成功匹配股票代码: %d/%d\n', matched_count, length(current_codes));
            fprintf('使用昨天权重作为换手率计算基准（按代码匹配）\n');
        else
            fprintf('警告: 昨天权重维度(%d)与代码数量(%d)不匹配，使用初始权重作为基准\n', length(yesterday_weight), length(yesterday_codes));
            turnover_baseline = initial_weight;
        end
    else
        fprintf('未找到昨天权重文件或代码文件，使用初始权重作为换手率计算基准\n');
    ...
    if nan_count > 0
        fprintf('警告: score_stock中有 %d 个NaN值，将其替换为0\n', nan_count);
    ...
    if inf_count > 0
        fprintf('警告: score_stock中有 %d 个Inf值，将其替换为0\n', inf_count);
    ...
    fprintf('目标函数设置：分数权重=%.2f, 夏普比率权重=%.2f\n', alpha, beta);
    ...
    try
        ...
        fprintf('  V的最小特征值: %.6f\n', min_eig);
        if min_eig <= 0
            fprintf('  警告: 协方差矩阵V不是正定矩阵，最小特征值: %.6f\n', min_eig);
        end
    catch ME
        fprintf('  错误: 无法计算协方差矩阵V的特征值: %s\n', ME.message);
    end
    ...
    fprintf('风险组件 (x''Vx*252): %.6f\n', risk_component);
    
    if risk_component < 0
        fprintf('警告: 风险组件为负数: %.6f\n', risk_component);
    end
    ...
    if isnan(objective_value) || isinf(objective_value)
        fprintf('错误: 目标函数值为NaN或Inf\n');
        fprintf('alpha: %.6f, beta: %.6f\n', alpha, beta);
        fprintf('score_component: %.6f\n', score_component);
        fprintf('sharpe_component: %.6f\n', sharpe_component);
    end
    ...
    if abs(initial_sum - 1.0) > 1e-6
        fprintf('警告: 初始点权重和(%.6f)偏离1.0，进行归一化调整\n', initial_sum);
        x0 = x0 / initial_sum;
        % 重新应用边界约束
        x0 = max(min(x0, ub), lb);
        % 再次归一化以确保权重和为1
