function df_score = score_zz1800_stockpool_processing(df_score, df_hs300, df_zz500, df_zz1000, dbc, data_date)
% score_zz1800_stockpool_processing - 中证1800股票池评分处理
% 筛选沪深300、中证500和中证1000股票池，并对评分进行标准化处理
%
% 输入:
%   df_score - 原始评分数据表
%   df_hs300 - 沪深300指数数据
...
%   df_score - 处理后的评分数据表
    ...
    % 合并并去重，相当于Python中的set union操作
    code_list_final = unique([code_list_hs300; code_list_zz500; code_list_zz1000]);
    
    % 筛选在股票池中的数据，相当于Python中的isin操作
    is_in_pool = ismember(df_score.code, code_list_final);
    df_score = df_score(is_in_pool, :);
    
    % 对final_score进行标准化：(x - mean) / std
